...
export class CustomerBusiness {
  ...
  customer = async (input: CustomerInputDTO) => {
    try {
      ...
      const verificationEmail = await this.customerData.findCustomerByEmail(
        email
      );
      ...
      const verificationDocument =
        await this.customerData.findCustomerbyDocument(document);
      ...
      const verificationBirthdate = new Date().toISOString().split("T")[0];
